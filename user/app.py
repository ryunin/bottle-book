...
def add():
    ...
    パスワード: <input type="text" name="passwd" value=""/><br/>
    email: <input type="text" name="email" value=""/><br/>
    氏: <input type="text" name="user_shi" value=""/><br/>
    名: <input type="text" name="user_mei" value=""/><br/>
    ...
    if request.forms.get('next') == 'back':
        ...
        with get_connection() as con:
            with con.cursor() as cur:
                cur.execute("SELECT COUNT(*) FROM book_user WHERE user_id = %s", (user_id,))
                count = cur.fetchone()[0]
                if count > 0:
                    # User already exists, update the record
                    sql = """UPDATE book_user 
                             SET passwd = %s, email = %s, user_shi = %s, user_mei = %s 
                             WHERE user_id = %s"""
                    val = (passwd, email, user_shi, user_mei, user_id)
                    cur.execute(sql, val)
                else:
                    ...
                    val = (user_id, passwd, email, user_shi, user_mei)
                    cur.execute(sql, val)
            con.commit()
        redirect('/add')
